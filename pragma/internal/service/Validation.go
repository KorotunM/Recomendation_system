...
func ValidateInput(city, specialty, score string) models.Errors {
	...
	if specialty != "" && !rusLetters.MatchString(specialty) {
		errors.SpecialtyError = "Профессия должна содержать только русские буквы, пробелы и тире."
	}
...
func HasErrors(errors models.Errors) bool {
	return strings.TrimSpace(errors.CityError) != "" ||
		strings.TrimSpace(errors.SpecialtyError) != "" ||
